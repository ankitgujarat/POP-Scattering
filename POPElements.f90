...
open(202,file='G:\Charge Transport Theory\Calculations\Dir_res\qptz.kpt',status='old',iostat=ios,action='read',form='formatted')
if (ios/=0) then
...
do i=1,nqs 
  ...
  call popelements(qps(:,i),zeff,epsil,atmpos,disp,freq,nat,epmatl,fac)
end do
close(199)
close(200)
...
!**********************************************************************************************************
!**********************************************************************************************************
subroutine popelements(q,zeff,epsil,atmpos,disp,freq,nat,epmatl,fac)
...
implicit none
integer :: nat
real :: q(3),zeff(3,3,nat),epsil(3,3),atmpos(3,nat),disp(3,nat,3*nat),freq(3*nat),epmatl(3*nat),fac
!
...
do mode = 1,3*nat
  ...
  if (freq(mode)> 0.2) then
    do natm = 1,nat
        ...
        do lt=1,3
            qz(lt)=qz(lt)+(q(1)*zeff(lt,1,natm))+(q(2)*zeff(lt,2,natm))+(q(3)*zeff(lt,3,natm))
        end do
        ...
        qze = qze+qze2
        !print *,qze,qze2,mode
        !qze = qze*fac*cmplx(1)*(((2*freq(mode))**(-0.5))/qeq)*exp(-qeq/4)
        !epmatl(mode) = epmatl(mode)+abs(qze)
    end do
    !qze3 = qze*fac*cmplx(1)*(((2*freq(mode))**(-0.5))/qeq)*exp(-qeq/4)
    !Cannot figure out the origin of the term exp(-qeq/4) 
    qze3 = (qze/qeq)*((2*freq(mode))**(-0.5))*fac*cmplx(1)
